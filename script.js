const root = document.documentElement;
  document.getElementsByClassName('inp')[0].addEventListener('input',()=>{
    let span_props = document.getElementsByClassName('txt')[0].getBoundingClientRect();
    ...
    for (let index = 0; index < document.getElementsByClassName('eye').length; index++) {
      ...
      let orgin_x = rect.x + rect.width/2;
      let orgin_y = rect.y + rect.width/2;
      let spanX = span_props.x+span_props.width; // X-coordinate relative to the viewport
      let spanY = span_props.y + span_props.height; // Y-coordinate relative to the viewport
      let angle = Math.atan2((spanY - orgin_y) , (spanX - orgin_x)) - Math.PI/4;
